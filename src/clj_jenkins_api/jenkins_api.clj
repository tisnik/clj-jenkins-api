...
(defn post-command
  ...
  (let [url (str (job-name->url (update-jenkins-url jenkins-url
                                                    jenkins-basic-auth)
                                job-name) "/" command)]
    (log "URL to use: " url)
...
(defn read-job-results
  ...
  (let [url (str (job-name->url jenkins-url job-name)
                 "/lastSuccessfulBuild/artifact/results.json")]
...
(defn start-job
  ...
  (job-related-command jenkins-url
                       jenkins-auth
                       include-jenkins-reply?
                       job-name
                       "build"))
...
(defn replace-placeholder
  ...
  (if value
    (clojure.string/replace string
                            (str "<placeholder id=\"" placeholder-name "\" />")
                            value)
    string))
...
(defn get-template
  ...
  (slurp (if metadata
           (str metadata-directory "/template_with_metadata.xml")
